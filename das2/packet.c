...
struct parse_pktdesc_stack{
    ...
    char errorMsg[XML_ELEMENT_NAME_LENGTH];
...
void PktDesc_parseStart(void *data, const char *el, const char **attr) {
	...
	if ( strcmp( el, "properties" )==0 ) {
		for (i=0; attr[i]; i+=2) {
			if( (pColon = strchr(attr[i], ':')) != NULL){
				...
				Desc_setProp(pStack->currentDesc, sType, sName, attr[i+1] );
			}
			else{
				Desc_setProp(pStack->currentDesc, "String", attr[i], attr[i+1]);
			}
		}
		return;
	}
	...
	if ( strcmp( el, "packet" )==0 ) {
		pStack->currentDesc = (Descriptor*)(pStack->pd);
		return;
	}
	...
	if( strcmp( el, "x" ) !=0    && strcmp( el, "y" )!=0 && 
		...
		pStack->errorCode= 18;
		sprintf(pStack->errorMsg, "Unsupported tag in <packet> element: %s\n",el);
		return;		
	}
	...
	PlaneDesc* pPlane = new_PlaneDesc_pairs((Descriptor*)pStack->pd, pt, attr);
	pStack->pd->planes[pStack->pd->uPlanes] = pPlane;
	pStack->pd->uPlanes += 1;
	
	pStack->currentDesc = (Descriptor*)pPlane;
}
...
PktDesc* new_PktDesc_xml(DasBuf* pBuf, Descriptor* pParent, int nPktId)
{
	...
	if ( !p ) {
		das2_error(18, "Couldn't create XML parser\n" );
		return NULL;
	}
	...
	if ( !nParRet) {
        das2_error(22, "Parse error at offset %ld:\n%s\n",
	...
	if ( stack.errorCode != 0 ) { 
        das2_error( stack.errorCode, stack.errorMsg );
	}
    
	if(pThis->uPlanes == 0){
		DasBuf_setReadOffset(pBuf, uPos);
		das2_error(18, "No data planes found! Here is the XML packet header:\n%s",
		           pBuf->pReadBeg);
		return NULL;
	}
...
bool PktDesc_equalFormat(const PktDesc* pPd1, const PktDesc* pPd2)
{
	...
	if(! Desc_equals((Descriptor*)pPd1, (Descriptor*)pPd2)) return false;
	...
	for(size_t u = 0; u<pPd1->uPlanes; u++){
		...
		if(pPlane1->sGroup && !(pPlane2->sGroup)) return false;
		if(!(pPlane1->sGroup) && pPlane2->sGroup) return false;
		if(pPlane1->sGroup && pPlane2->sGroup && 
		   strcmp(pPlane1->sGroup, pPlane2->sGroup) != 0) return false;
		
		if(! Desc_equals((Descriptor*)pPlane1, (Descriptor*)pPlane2) ) return false;
	}
...
int PktDesc_addPlane(PktDesc* pThis, PlaneDesc* pPlane)
{
	if(pThis->uPlanes >= MAXPLANES){
		das2_error(18, "Too many planes, limit is %d\n", MAXPLANES );
		return -1;
	}
	...
	if(pPlane->planeType == X){
		if(PktDesc_getNPlanesOfType(pThis, X) > 1){
			das2_error(18, "There can only be a maximum of 2 X-planes in a packet");
			return -1;
		}
	}
	if(pPlane->planeType == YScan){
		if(PktDesc_getNPlanesOfType(pThis, Z) > 0){
			das2_error(18, "YScan and Z planes cannot be present in the same packet");
			return -1;
		}
	}
	if(pPlane->planeType == Z){
		if(PktDesc_getNPlanesOfType(pThis, YScan) > 0){
			das2_error(18, "Z and YScan planes cannot be present in the same packet");
			return -1;
		}
	}
	...
	pPlane->base.parent = (Descriptor*)pThis;
...
ErrorCode PktDesc_copyPlanes(PktDesc* pThis, const PktDesc* pOther)
{
	...
	if(pThis->uPlanes > 0)
		return das2_error(18, "ERROR: Can't use copyPlanes here, packet type "
...
bool PktDesc_validate(PktDesc* pThis ) {
   /* Make sure the dependent planes are present */
	if((PktDesc_getNPlanesOfType(pThis, Y) > 0) && 
		...
		das2_error(18, "In packet type %02d, Y planes are present without an X "
				     "plane", pThis->id);
		return false;
	}
	...
	if((PktDesc_getNPlanesOfType(pThis, YScan) > 0) && 
		(PktDesc_getNPlanesOfType(pThis, X) == 0)){
		das2_error(18, "In packet type %02d, YScan planes are present without an X "
				     "plane", pThis->id);
		return false;
	}
	...
	if((PktDesc_getNPlanesOfType(pThis, Z) > 0) && 
		...
		das2_error(18, "In packet type %02d, Z planes are present without a Y "
				     "plane", pThis->id);
		return false;
	}
	return true;
...
int PktDesc_getId(const PktDesc* pThis){
	...
	if(pThis->id < 0 || pThis->id > 99)
		return das2_error(17, "Packet Descriptor has Invalid packet ID: %d ", 
		                  pThis->id);
	return pThis->id;
}
...
plane_type_t PktDesc_getPlaneType(const PktDesc* pThis, int iPlane)
{
	if(iPlane < 0 || iPlane >= MAXPLANES)
		das2_error(18, "ERROR: Invalid value for the plane index %d", iPlane);
...
int PktDesc_getPlaneNumByName(PktDesc* pThis, const char * name, plane_type_t planeType )
{
    ...
    for ( iplane=0; iplane<pThis->uPlanes; iplane++ ) {
        if ( planeType==pThis->planes[iplane]->planeType && 
				 strcmp( name, pThis->planes[iplane]->sGroup )==0 ) {
            return iplane;
        }
    }
    return -1;
...
PlaneDesc* PktDesc_getPlaneByName(PktDesc* pThis, const char* name)
{
	...
	for(iplane=0; iplane<pThis->uPlanes; iplane++ ) {
		if(strcmp( name, pThis->planes[iplane]->sGroup )==0 ) {
...
PlaneDesc* PktDesc_getXPlane(PktDesc* pThis ) {
	int iPlane = PktDesc_getPlaneNum(pThis, X, 0);
...
PlaneDesc* PktDesc_getPlane(PktDesc* pThis, int iplane ) {
    if ( iplane<0 || iplane>pThis->uPlanes ) {
        das2_error(18, "invalid plane number, iplane=%d\n", iplane );
    }
...
ErrorCode PktDesc_encode(const PktDesc* pThis, DasBuf* pBuf)
{
	...
	if( (nRet = DasBuf_printf(pBuf, "<packet>\n")) != 0) return nRet;
	if( (nRet = Desc_encode((Descriptor*)pThis, pBuf, "  ")) != 0) return nRet;
	
	/* Independent Planes are not serialized with a name attribute, so have to
	   check for that */
	bool bYDep = true; 
	int iplane;	
	for(iplane=0; iplane < pThis->uPlanes; iplane++ ){
		if(pThis->planes[iplane]->planeType == Z){
			bYDep = false;
			break;
		}
	}
	...
	for(iplane=0; iplane < pThis->uPlanes; iplane++ ) {
		...
		case X:
			nRet = PlaneDesc_encode(pThis->planes[iplane], pBuf, "  ", false);
			break;
		case Y:
			nRet = PlaneDesc_encode(pThis->planes[iplane], pBuf, "  ", bYDep);
			break;
		case Z:
		case YScan:
			/* Z and YScan planes are always dependent data */
			nRet = PlaneDesc_encode(pThis->planes[iplane], pBuf, "  ", true);
			break;
		default:
			return das2_error(18, "Code change detected in PktDesc_encode");
			break;
		}
...
ErrorCode PktDesc_decodeData(PktDesc* pThis, DasBuf* pBuf){
	...
	if(DasBuf_unread(pBuf) < uRecBytes){
		return das2_error(18, "For packet type %02d, %d bytes expected in each "
...
ErrorCode PktDesc_encodeData(const PktDesc* pThis, DasBuf* pBuf){
	...
	if((uEnd - uBeg) != uRecBytes) 
		return das2_error(18, "Partial packet written expected output %zu bytes, "
...
void PktDesc_dump(const PktDesc* pThis, FILE* file)
{
	...
	for ( i=0; i<pThis->uPlanes; i++ ) {
		p= pThis->planes[i];
		DasEnc_toString(p->pEncoding, sEncoding, 24);
		DasBuf_printf(pBuf, "%5d: %7d %6d %6d %s %s\n",
                i, uOffset, p->pEncoding->nWidth * p->uItems, p->uItems,
                sEncoding, PlaneType_toString(p->planeType) );
