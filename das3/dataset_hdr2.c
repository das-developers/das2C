 ...
 * values always were. */
static DasVar* _serial_setFromAry(DasAry* pAry, int nExtRank, const int8_t* pMap)
{
...
#define _MAP2(I,J) 2, (int8_t[VARIDX_MAX]){I,J,-3,-3,-3,-3,-3,-3}
		...
		case YScan:
			...
			if(!bAddedYTags){
				...
				if( _serial_isWaveform(pPlane) ){
					...
					*/
					pVar = (DasVar*)new_DasVarBin(pReference, '+', pOffset);
