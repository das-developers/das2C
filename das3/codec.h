...
#include <das3/encoding.h>  /* <-- only to get DASENC_FMT_LEN */
...
typedef struct das_codec {
	...
	ubyte nSep;
	char sSepSet[VARIDX_MAX];  /* Split strings on these chars by rank */
 ...
 * @param nLevels the number of run-terminator levels (1..VARIDX_MAX-1)
 ...
 * @param aRagIdx receives the index positions; must hold VARIDX_MAX entries
