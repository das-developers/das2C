...
build:$(BD) $(BD)/$(TARG) $(BUILD_UTIL_PROGS) $(BD)/_das2.so das2/*.py
	cp das2/__init__.py $(BD)/das2
	cp das2/dastime.py $(BD)/das2
	cp das2/toml.py $(BD)/das2
...
test: $(BD) $(BD)/$(TARG) $(BUILD_TEST_PROGS)
	$(BD)/TestUnits
	env PYTHONPATH=$(PWD)/$(BD) test/test_dastime.py 
	env PYTHONPATH=$(PWD)/$(BD) test/das1_fxtime_test.sh $(BD)
	env PYTHONPATH=$(PWD)/$(BD) test/das2_dastime_test1.sh $(BD)
