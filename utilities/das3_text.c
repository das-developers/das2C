...
"\n"
"DESCRIPTION\n"
"   " PROG " is a filter.  It reads a dasStream on standard input (or from\n"
"   INFILE if given) and writes a dasStream to standard output.  Every data\n"
"   value encoded in binary is re-encoded as text; values already encoded as\n"
"   text pass through untouched.  The output is still a valid dasStream, so it\n"
"   re-parses, diffs, and passes das_verify just like the input.\n"
"\n"
